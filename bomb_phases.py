...
class Lcd(Frame):
    ...
    def setup(self):
        ...
        if (SHOW_BUTTONS):
            ...
            pygame.init()
            pygame.mixer.music.load("bomb_planted.mp3")
    ...
    def conclusion(self, success=False):
        ...
        if (success):
            image = PhotoImage(file="defused_screen.png")
            ...
            image = PhotoImage(file="bmb_exp.png")
            self._lscroll["image"] = image
            self._lscroll.image = image
            self._lscroll.grid(row=0, column=0, columnspan=3, sticky=EW)
            pygame.mixer.music.load("bomb_explode.mp3")
            pygame.mixer.music.play(1)
            print("EXPLODE!")
...
class Wires(PhaseThread):
    ...
    def _check_wrong_state(self):
        #return T if the changed toggle is in the wrong position
        #F otherwise
